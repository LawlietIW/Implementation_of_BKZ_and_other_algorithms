...
def swapG(k, kmax, Bm, Mym, B_norms, epsilon = 0.05):
    ...
    if k > 1:
        for j in range(k-1):
            ...
            Mym[k-1,j], Mym[k,j] = Mym[k,j], Mym[k-1,j]
    ...
    if B < epsilon:
        ...
        # print(k)
        B_norms[k-1], B_norms[k] = B_norms[k], B_norms[k-1]
        # print("After:", B_norms)
...
def improvedLLL(Bm, delta = 3/4):
    ...
    return Bm[~np.all(Bm == 0, axis=1)]   #returns all non zero rows
            

if __name__ == "__main__":
    Bm = np.random.randint(500, size=(140,140))
    # print("Bm: ")
    # print(np.array2string(Bm, separator=', '))
    start = time.time()
    LLL = improvedLLL(Bm, 0.8)
